...
template<class Keys,class T>
int BTree<Keys,T>::isInternal(BNode<Keys,T>* p)	//node 1 con
{
	return p->hasLeft()||p->hasRight();
}
template<class Keys,class T>
int BTree<Keys,T>::isExternal(BNode<Keys,T>*p)	//node la'
{
...
BNode<Keys,T>* BTree<Keys,T>::insert(Keys key,T elem)
{
	...
	else
	{
		...
		while(p!=NULL)
		{
			...
			else if(key > p->getKey()) //nam ben cay con ben phai
			{	
				if(p->getRight()==NULL)
				{
					q->setParent(p);
...
void BTree<Keys,T>::remove(Keys key)
{
	...
	if(v==NULL)	return;
	if(v->hasLeft() && v->hasRight())//Co ca hai con
	{
		BNode<Keys,T> *first;
		int kt=0;
