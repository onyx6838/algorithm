...
class BTree
{
	...
	public:
		...
		int size(){return n;}
		int isInternal(BTreeNode<T> *v)//Node con
		{
			return v->hasLeft() || v->hasRight();
		}	
		int isExternal(BTreeNode<T> *v)//La
		{
			return (!v->hasLeft()) && (!v->hasRight());
		}
...
template<class T>
void BTree<T>::inOrder(BTreeNode<T>*v,void(*visit)(BTreeNode<T> *a))
{
	if(v!=NULL)
	{
...
template<class T>
void BTree<T>::preOrder(BTreeNode<T>*v,void(*visit)(BTreeNode<T> *a))
{
	if(v!=NULL)
	{	
		visit(v);
...
BTreeNode<T>* BTree<T>::insert(BTreeNode<T> *parent,T elem)
{
	...
	{
		if(!parent->hasLeft()) //sang nhanh trai
			parent->setLeft(p);
		else
		{
			if(!parent->hasRight()) //sang nhanh phai
				parent->setRight(p);
			else	
				return NULL;
		}
//		if(LeftRight==1) (ktra = bien truyen vao xem da~ co node trai/phai chua)
//			parent->setRight(p);
//		else
//			parent->setLeft(p);	
	}
