...
{ 
    ...
    { 
        ...
        { 
            ...
            // recur over the rest 
            suggestionsRec(root->children[i], _currPrefix); 
        } 
    } 
}
...
{ 
    ...
    if (isWord && isLast) 
    { 
    cout<<"find keyword in trie"<<endl;
        cout << query << endl; 
        return -1; 
    }
    ...
    // matching character. 
		if(!isLast)
		{
			...
			for(int i=0;i<10;++i)
				...
				{
					suggestionsRec(pCrawl->children[i],string(prefix + (char)(48 + i)));  
				}
			return 1;
		}
} 
...
//    insert(root, "helps"); 
    ...
    int comp; 
	comp = printAutoSuggestions(root, "190");
	
    if (comp == -1) 
        cout << "No other strings (except prefix in trie) found with this prefix\n"; 
    else if (comp == 0) 
