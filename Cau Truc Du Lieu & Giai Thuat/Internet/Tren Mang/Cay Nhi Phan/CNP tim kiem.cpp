Xóa 1 node b?t kì trong cây nh? phân tìm ki?m
Node X là node c?n xóa:
N?u X:
1. Node lá: xóa bình thu?ng - b?i vì lúc này node x dang là node t? do
(không t?n t?i cây con trái và cây con ph?i)
2. Node có 1 con: tru?c khi xóa node x - chúng ta ph?i c?p nh?t m?i liên k?t
gi?a node cha c?a node x v?i node con(con trái ho?c con ph?i) c?a node x 
3. Node có 2 con: tru?c khi h?y thì ta ph?i di tìm 1 cái node th? m?ng cho node c?n xóa
- tìm node th? m?ng ph?i d?m b?o quy t?c cây nh? phân tìm ki?m
+ Tìm node trái nh?t c?a cây con ph?i(cây con ph?i c?a cái node c?n xóa)
+ Tìm node ph?i nh?t c?a cây con trái(cây con trái c?a cái node c?n xóa)
